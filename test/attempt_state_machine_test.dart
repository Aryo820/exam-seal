...
void main() {
  ...
  test(
    'violation on locked attempt re-locks immediately without a new grant of chances',
    () {
  ...
  test(
    'supervisor can continue or end a locked attempt; counter and history stay',
    () {
      final machine = AttemptStateMachine(initialViolationCount: 3);
      machine.restore(AttemptState.locked);
  ...
  test(
    'active attempt cannot be overwritten by a new scan; ended can only be repeated by supervisor',
    () {
      final machine = AttemptStateMachine(initialViolationCount: 0);
      machine.start();
  ...
  test(
    'process death on an active attempt moves to recoveryPending, not back into the exam',
    () {
      final machine = AttemptStateMachine(initialViolationCount: 0);
      machine.start();
      machine.registerViolation(reason: 'Aplikasi ditinggalkan');
  ...
  test(
    'session identity mismatch is rejected so a new scan cannot swap active session data',
    () {
      final machine = AttemptStateMachine(initialViolationCount: 0);
