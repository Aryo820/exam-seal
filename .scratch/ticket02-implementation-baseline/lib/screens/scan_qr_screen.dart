...
class _ScanQrScreenState extends State<ScanQrScreen> {
  ...
  void _detected(BarcodeCapture capture) {
    if (_busy || _message != null || !mounted) return;
    final codes = capture.barcodes.where(
      (b) => b.format == BarcodeFormat.qrCode,
    );
    if (codes.isEmpty) return;
    _handleCodes(codes.map((b) => b.rawValue ?? ''));
  }

  void _handleCodes(Iterable<String> values) {
    ...
    setState(() => _busy = true);
    unawaited(_deliver(session));
  }

  Future<void> _deliver(ExamSession session) async {
    try {
      await widget.onSession?.call(session);
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  onPressed: _busy || !_supported ? null : _pickImage,
