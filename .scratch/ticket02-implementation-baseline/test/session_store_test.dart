...
void main() {
  ...
  test('violations persist so reopening the app cannot reset the counter', () async {
    final store = await newStore();
    await store.saveSession(session);
    ...
    await store.setAttemptState(attemptId, AttemptState.locked, violationCount: 3);
  ...
  test('retention only deletes ended attempts older than seven days', () async {
    ...
    await store.setAttemptState(activeId, AttemptState.active, violationCount: 1);
    ...
    await store.setAttemptEndedAt(endedId, DateTime.now().subtract(const Duration(days: 10)));
    await store.setAttemptUpdatedAt(endedId, DateTime.now().subtract(const Duration(days: 10)));
  ...
  test('a failed write surfaces an actionable error instead of a trusted new state', () async {
    final db = await factory.openDatabase(inMemoryDatabasePath);
    openDbs.add(db);
  ...
  test('repeat by supervisor creates a fresh attempt while old history stays', () async {
    final store = await newStore();
    await store.saveSession(session);
