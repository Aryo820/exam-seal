...
void main() {
  test(
    'QR menolak kebijakan dan bahan verifier yang tidak didukung tanpa melempar',
    () async {
      ...
      for (final change in <Map<String, dynamic>>[
        ...
        {'pinVerifier': 'A' * 6000},
        {'iterations': 1},
        {'initialViolationLimit': 99},
      ]) {
        final result = decodeScan([
          jsonEncode({...original, ...change}),
        ]);
        expect(result.session, isNull, reason: change.keys.join(','));
  ...
  test(
    'scan rejects invalid input and decodes one supported ExamSeal session',
    () {
  ...
  test(
    'QR v2 round-trips session and verifier material; raw PIN never appears',
    () async {
  ...
  test(
    'QR payload with same id but different verifier is flagged as a conflict',
    () async {
      final session = await sessionWithPin();
      final encoded = jsonEncode(
        jsonDecode(encodeSessionQr(session)) as Map<String, dynamic>
          ..['pinVerifier'] = base64Encode(List.filled(32, 7)),
      );
