...
class ExamSessionController {
  ...
  Future<TeacherSessionCreation> createTeacherSession({
    ...
    final session = ExamSession(
      ...
      sessionCode: PinService.generateSessionCode(seedName: examName),
      examName: examName,
      formUrl: formUrl,
  ...
  Future<List<ExamSession>> listTeacherSessions() => _store.listSessions();

  /// PIN untuk ditampilkan guru di HP pembuat setelah local_auth.
  Future<String?> readTeacherPin(String sessionId) => _secrets.readPin(sessionId);
  ...
  Future<({ScanImportRoute? route, String? error})> importScannedSession(
    ...
    if (existingAttempts.isNotEmpty) {
      ...
      await _store.saveSession(scanned);
    }
    ...
    final ended = existingAttempts.any((a) => a.state == AttemptState.ended);
    if (ended) {
  ...
  Future<ReadinessReport> assessReadiness(ExamSession session) async {
    ...
    try {
      // Tulis-raciik ringan: pastikan store masih hidup.
      await _store.loadSession(session.sessionId);
    ...
    final protectionReady =
        protection != null && await protection.isReady();

    return ReadinessReport(
      qrValid: qrValid,
      urlValid: urlValid,
      storageWritable: storageWritable,
      screenProtectionReady: protectionReady && protection.screenProtectionReady,
      notificationControlReady:
  ...
  Future<StartAttemptResult> startStudentAttempt(ExamSession session) async {
    ...
    if (!readiness.allMandatoryPassed) {
      return StartAttemptResult(
        started: false,
        reason: 'Perangkat belum siap. Minta bantuan pengawas atau gunakan ujian alternatif.',
      );
    }
    ...
    if (protection == null || !await protection.activate()) {
      return StartAttemptResult(
        started: false,
        reason: 'Proteksi perangkat gagal diaktifkan. Ujian tidak dapat dimulai.',
      );
    }
    ...
    try {
      ...
      final attemptId = await _store.startAttempt(session, attemptNumber: attemptNumber);
  ...
  Future<StoredAttempt?> loadLastEndedAttempt() async {
    ...
    // pengulangan.
    final rows = await _store.loadAttemptsFor((await _lastSessionIdWithEnded()) ?? '');
    if (rows.isEmpty) return null;
    return rows.lastWhere((a) => a.state == AttemptState.ended,
        orElse: () => rows.last);
  }
  ...
  Future<({AuthorizationResult authorization, bool settingsRestored})>
    ...
    if (!ok) {
      return (
        authorization: const AuthorizationResult(authorized: false, wrongPin: true),
        settingsRestored: false,
      );
    }
...
class _StubProtection implements ExamProtectionBridge {
  ...
  @override
  Future<bool> isReady() async => screenProtectionReady && notificationControlReady;
