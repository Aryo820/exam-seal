...
class SessionStore {
  ...
  Future<void> saveSession(ExamSession session) => _guard('menyimpan sesi', () async {
  ...
  Future<List<ExamSession>> listSessions() => _guard('memuat daftar sesi', () async {
        final rows = await _db.query('sessions', orderBy: 'created_at DESC');
        return rows.map(_sessionFromRow).toList();
      });

  Future<ExamSession?> loadSession(String sessionId) => _guard('memuat sesi', () async {
  ...
  Future<String> startAttempt(ExamSession session, {required int attemptNumber}) =>
      _guard('memulai attempt', () async {
        final now = DateTime.now();
        final attemptId = 'attempt-${session.sessionId}-$attemptNumber-${now.millisecondsSinceEpoch}';
  ...
  Future<StoredAttempt?> loadCurrentAttempt() => _guard('memuat attempt tersimpan', () async {
        final rows = await _db.query(
          'attempts',
          where: 'state IN (?, ?, ?)',
          whereArgs: [
  ...
  AttemptState _stateFromName(String name) => AttemptState.values
      .firstWhere((s) => s.name == name, orElse: () => AttemptState.preExam);

  Future<void> setAttemptState(String attemptId, AttemptState state,
  ...
  Future<void> setAttemptEndReason(String attemptId, String reason) =>
      _guard('menyimpan alasan pengakhiran', () async {
        await _db.update(
          'attempts',
          {'ended_reason': reason, 'ended_at': DateTime.now().millisecondsSinceEpoch},
          where: 'attempt_id = ?',
          whereArgs: [attemptId],
        );
      });
      ...
      _guard('menyimpan status proteksi', () async {
        await _db.insert(
          'protection_states',
          {
            ...
            'notification_protection_state':
                notificationProtectionActive ? 'active' : 'inactive',
  ...
  Future<bool> hasPendingRestore() => _guard('memeriksa pemulihan tertunda', () async {
        final rows = await _db.query(
          'protection_states',
      ...
      _guard('memuat status percobaan PIN', () async {
        ...
        return (
          ...
          lockedUntil: rows.first['locked_until'] == null
              ? null
              : DateTime.fromMillisecondsSinceEpoch(rows.first['locked_until'] as int),
        );
      });
  ...
  Future<List<String>> runRetention({required DateTime now}) =>
      _guard('menjalankan retensi', () async {
        ...
        for (final id in ids) {
          await _db.delete('session_events', where: 'attempt_id = ?', whereArgs: [id]);
          await _db.delete('supervisor_actions', where: 'attempt_id = ?', whereArgs: [id]);
          await _db.delete('protection_states', where: 'attempt_id = ?', whereArgs: [id]);
          await _db.delete('pin_attempt_status', where: 'attempt_id = ?', whereArgs: [id]);
          await _db.delete('attempts', where: 'attempt_id = ?', whereArgs: [id]);
        }
        // Sesi tanpa attempt tersisa juga dihapus bila sudah melewati
        // masa retensi; sesi yang masih dibutuhkan attempt aktif tetap.
        final sessions = await _db.query('sessions', columns: ['session_id']);
        for (final row in sessions) {
          ...
          if (remaining.isEmpty) {
            await _db.delete('sessions', where: 'session_id = ?', whereArgs: [sessionId]);
          }
        }
        return ids;
      });
