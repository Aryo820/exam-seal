...
class PinVerificationMaterial {
  const PinVerificationMaterial({
    required this.salt,
    required this.verifier,
  });

  factory PinVerificationMaterial.fromSessionParts(String salt, String verifier) =>
...
class PinAttemptState {
  ...
  factory PinAttemptState.fromJson(Map<String, dynamic> json) => PinAttemptState(
...
abstract final class PinService {
  ...
  static String generateSessionCode({required String seedName}) {
    ...
    final prefix = (letters.isEmpty ? 'EXM' : letters.substring(0, 3)).padRight(3, 'X');
    final alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
    final random = List.generate(4, (_) => alphabet[_secure.nextInt(alphabet.length)]);
  ...
  static Future<PinVerificationMaterial> createVerificationMaterial(String pin) async {
  ...
  static Future<bool> verifyPin(String pin, PinVerificationMaterial material) async {
